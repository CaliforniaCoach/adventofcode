...
MATERIALS = {'ore': 0, 'clay': 1, 'obsidian': 2, 'geode': 3}
...
while words != []:
	if words[0] == 'Blueprint':
		...
		blueprints[blueprint_no] = [[0] * len(MATERIALS) for i in MATERIALS]
		del words[0:2]
	elif words[0] == 'Each':
		material = MATERIALS[words[1]]
		del words[0:3]
		while words != [] and re.fullmatch('\\d+', words[1]):
			blueprints[blueprint_no][material][MATERIALS[re.sub('([^\\.]+)\\.?', '\\1', words[2])]] = int(words[1])
			del words[0:3]
	else:
		raise ValueError
...
def collect(blueprint, robots, materials, minute, next_to_build):
	...
	for robot in range(NUM_MATERIALS):
		new_materials[robot] += robots[robot]
	if minute >= max_minutes:
		return new_materials[GEODE]

	# If robot can be built: build it
	if next_to_build != None:
		materials_after_build = list(new_materials)
		for required_material, amount in enumerate(blueprint[next_to_build]):
	...
	max_geode = 0
	for robot, required_materials in enumerate(blueprint):
		for required_material, amount in enumerate(required_materials):
			if amount > 0 and new_robots[required_material] <= 0:
				break
		else:
